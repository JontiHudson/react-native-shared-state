...
export default class SharedMap extends SharedState {
  ...
  remove(id, _update = true) {
    const { _map, _organisedArrays } = this.state;

    const currentElement = _map[id];
    if (currentElement) {
      delete _map[id];

      for (const name in _organisedArrays) {
        _organisedArrays[name] = _organisedArrays[name].filter(
          (element) => element[this._key] !== id,
        );
      }
    }
    if (_update) {
  ...
  _organiseArray(newElements, organiseFunction, organisedArray) {
    ...
    SharedMap._sortAndFilterNewElements(
      newElements,
      organiseFunction,
      tempArray,
    );
