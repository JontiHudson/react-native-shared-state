...
export default class SharedState {
  ...
  _registerHook(id, propKey, setProp) {
    ...
    const onEmit = emittedState => {
      if (Object.keys(emittedState).includes(propKey)) {
        setProp(emittedState[propKey]);
      }
    };

    this._register.set(id, onEmit);
  }
  ...
  useState(propKey) {
    ...
    const currentValue = this.getState()[propKey];

    const [prop, setProp] = useState(currentValue);
