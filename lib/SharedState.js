...
export default class SharedState {
  ...
  reset() {
    ...
    const { defaultState, currentState } = this._states;
    const resetState = {};
  ...
  _validateProp(prop, validator) {
    ...
    if (Array.isArray(validator)) {
      for (const validatorElement of validator) {
  ...
  _isValid(prop, validatorProp) {
    ...
    if (Array.isArray(prop) && Array.isArray(validatorProp)) {
      ...
      for (const propElement of prop) {
        for (const validatorElement of validatorProp) {
          try {
    ...
    if (typeof validatorProp === 'object' || typeof validatorProp === 'function') {
  ...
  register(component, updateKeys) {
    ...
    const updateKeysArray = Array.isArray(updateKeys) ? updateKeys : [updateKeys];

    function onUpdate(updatedState) {
      if (Object.keys(updatedState).some((key) => updateKeysArray.includes(key))) {
  ...
  _registerHook(id, updateKeys, reRender) {
    this._debugger({ registerHook: { id, updateKeys } });

    function onUpdate(updatedState) {
      const update = Object.keys(updatedState).some((key) => updateKeys.includes(key));
  ...
  async useStorage(storeName, options = {}) {
    ...
    try {
      ...
      if (this._saveOnBackground) {
        ...
        AppState.addEventListener('change', this._handleAppBackgrounded.bind(this));
      }
  ...
  save(storeName = this._asyncStoreName) {
    try {
      if (!storeName) throw new Error('Store not initalised');
      ...
      if (this._encryptionKey) {
        stateString = CryptoJS.AES.encrypt(stateString, this._encryptionKey).toString();
      }
